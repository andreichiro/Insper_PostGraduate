...
def add_features(cleaned_dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    featured["avg_charge_per_month"] = featured["TotalCharges"] / (
        featured["tenure"] + 1
    )
    logger.info("add_features: added avg_charge_per_month")
    return featured
...
def add_split_column(
    ...
    Falls back to random sampling otherwise (or for small test datasets).
    """
...
def fit_encoders(
    ...
    Uses ``OrdinalEncoder`` (not ``LabelEncoder``) — designed for features,
    handles unseen categories natively with ``unknown_value=-1``.
    """
